...
namespace CEDBCN_MonitorIT.Models
{
    public class ConsultaEstadoRobots
    {
        ...
        public List<Estado_Robot> RecuperarTodos()
        {
            Conectar();
            List<Estado_Robot> robots = new List<Estado_Robot>();

            SqlCommand com = new SqlCommand("SELECT IDRobot,Nombre,Estado,F_UltimaActualizacion,TipoRobot,RobotPadre,Posicion,ErroresDia,ErroresCorregidos,TotalAP_PLAT,TotalEjecuciones,Observaciones from dbo.Estado_Robot order by IDRobot ASC", con);
            con.Open();
            SqlDataReader registros = com.ExecuteReader();
            while (registros.Read())
            {
                Estado_Robot robot = new Estado_Robot
                {                  
                    ...
                    TipoRobot = registros["TipoRobot"].ToString(),
